...
describe("Bound Market Contracts", () => {
  ...
  it("Updates the price", async () => {
    const tx = await program.methods
      ...
      .accounts({
        orderbook: orderbookPda,
        authority: provider.wallet.publicKey,
        oracle: oracle.publicKey, // Add oracle account
      })
      .rpc();
  ...
  it("Matches a position order", async () => {
    ...
    const tx = await program.methods
      ...
      .accounts({
        ...
        liquidityOrder: breakoutOrderPda, // Match with a breakout order for a StayIn position
  ...
  it("Cancels an order", async () => {
    ...
    const tx = await program.methods
      ...
      .accounts({
        ...
        ordersPda: stayInOrdersPda, // Use the correct orders collection with camelCase
      })
      .signers([trader1])
      .rpc();
  ...
  it("Processes expired orders", async () => {
    ...
    const tx = await program.methods
      ...
      .accounts({
        orderbook: orderbookPda,
        authority: provider.wallet.publicKey,
        orders: breakoutOrdersPda, // Use the orders collection
        clock: anchor.web3.SYSVAR_CLOCK_PUBKEY,
      })
      .rpc();
  ...
  it("Settles an order", async () => {
    ...
    const tx = await program.methods
      ...
      .accounts({
        orderbook: orderbookPda,
        trader: trader2.publicKey, // Use trader2 since they own the position order
        traderState: trader2State,
        order: positionOrderPda, // Use the position order that was matched
        authority: provider.wallet.publicKey,
        clock: anchor.web3.SYSVAR_CLOCK_PUBKEY,
      })
      .rpc();
